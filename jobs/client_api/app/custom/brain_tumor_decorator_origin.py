...
def main():
    ...
    train_data, train_dataloader, valid_data, valid_dataloader =  load_data(data_split, client_id, image_transform )
    ...
    @flare.train
    def train_model(model, loss_func, lr, epochs, image_datasets, image_dataloaders, input_model=None):
    ...
    def  evaluate(model, image_datasets, image_dataloaders, input_model=None):
        """Return the trained model and train/test accuracy/loss"""
        #if not do_training:
        #    return None, None
        model.load_state_dict(input_model.params)
    ...
    while flare.is_running():
        ...
        # call train method
        model= TumorNet()
        train_model(model, loss_func, 0.01, 3, image_datasets, image_dataloaders)
        metric = evaluate(input_weights=torch.load(PATH))
        print(f"Accuracy of the trained model on the test images: {metric} %")
